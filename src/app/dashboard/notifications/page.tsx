...
function UpgradeGate({ feature }: { feature: string }) {
  return (
    <div className="rounded-xl p-10 text-center" style={{ background: "var(--surface-1)", border: "1px solid var(--border)" }}>
      ...
      <p className="text-base font-semibold text-white mb-1">{feature} — Pro uniquement</p>
      <p className="text-sm mb-6" style={{ color: "var(--text-muted)" }}>
        Passez à la formule Pro pour envoyer des notifications illimitées à vos clients.
      </p>
      <Link
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex gap-1 mb-6 p-1 rounded-lg w-fit" style={{ background: "var(--surface-1)", border: "1px solid var(--border)" }}>
        {[
          ...
          >
            {icon}{label}
            {isStarter && (
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3 h-3 ml-0.5" style={{ color: "#f59e0b" }}>
                <rect x="3" y="11" width="18" height="11" rx="2" />
                <path d="M7 11V7a5 5 0 0 1 10 0v4" strokeLinecap="round" />
              </svg>
            )}
      ...
      {tab === "email" && (
        <>
          {isStarter ? (
            <UpgradeGate feature="Notifications email" />
          ) : (
            <>
      ...
      {tab === "push" && (
        <>
          {isStarter ? (
            <UpgradeGate feature="Notifications push" />
          ) : (
            <>
              <div className="rounded-xl p-5 mb-4" style={{ background: "var(--surface-1)", border: "1px solid var(--border)" }}>
