...
function ClientLoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result?.error) {
      ...
      if (rememberMe) {
        localStorage.setItem("fidco_client_remember", JSON.stringify({ expires: Date.now() + 30 * 24 * 60 * 60 * 1000 }));
      } else {
        sessionStorage.setItem("fidco_client_session", "1");
      }
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col px-4 pt-6 pb-10">
      <Link href="/login" className="inline-flex items-center gap-2 text-slate-400 hover:text-white text-sm transition-colors self-start mb-10">
        ← Retour
      </Link>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password"
                  className="w-full px-4 py-3.5 pr-12 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition-colors text-base"
                  placeholder="••••••••" />
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="/mot-de-passe-oublie" className="text-amber-400 hover:text-amber-300 text-xs font-medium transition-colors">
                Mot de passe oublié ?
              </Link>
            </div>
          ...
          <p className="text-slate-500 text-xs text-center mt-4">Pas de compte ? Scannez le QR code d&apos;un commerce.</p>
        </div>
      </div>
    </div>
  );
